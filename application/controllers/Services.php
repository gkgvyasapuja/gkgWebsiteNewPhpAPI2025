...
class Services extends CI_Controller {
	...
	public function add_Offering()
	{
		...
		$SpiritualName = $this->input->get('SpiritualName');
		...
		$q = $this->db->insert('Offerings', array('UserId' => $UserId,'FirstName' => $FirstName,'LastName' => $LastName, 'YearId' => $YearId, 'TempleId' => $TempleId, 'OfferingFile' => '/api/Offering/'.$Country.'/'.$State.'/'.$City.'/'.$fileName, 'InitiationType' => $InitiationType, 'InitiationYear' => $InitiationYear, 'Gender' => $Gender, 'Country' => $Country, 'State' => $State, 'City' => $City, 'TempleName' => $TempleName, 'OfferingType' => $OfferingType, 'SpiritualName' => $SpiritualName, 'CreatedOn' => date('Y-m-d H:i:s')));
	...
	public function fetch_User()
	{
		...
		$q = $this->db->select('Users.*, Offerings.TempleId,Offerings.OfferingType, Offerings.InitiationType, Offerings.InitiationYear , Offerings.SpiritualName,Offerings.TempleName,states.name as StateName ,countries.name as CountryName,cities.name as CityName')
		->where('UPPER(Email)', strtoupper($Email))
		->join('Offerings', ' Offerings.UserId = Users.id', 'left')
		->join('cities', ' cities.id = Users.CityId', 'left')
		->join('states', ' states.id = Users.StateId', 'left')
		->join('countries', ' countries.id = Users.CountryId', 'left')
		->where('Offerings.OfferingType', 'self')
		->order_by('Offerings.id', 'desc')
		// ->group_by('Offerings.id')
		->get('Users');
		} else {

		$q1 = $this->db->select('Users.*')
					->get('Users');
		$q = $this->db->select('Users.*, Offerings.TempleId, Offerings.OfferingType,Offerings.InitiationType, Offerings.InitiationYear , Offerings.SpiritualName,Offerings.OfferingFile, Offerings.TempleName,states.name as StateName ,countries.name as CountryName,cities.name as CityName')
		// ->where('UPPER(Email)', strtoupper($Email))
		->join('Offerings', ' Offerings.UserId = Users.id', 'left')
		->join('cities', ' cities.id = Users.CityId', 'left')
		->join('states', ' states.id = Users.StateId', 'left')
		...
		if($q->num_rows() > 0) {
			$response['pageNumber'] = 1;
			$response['pageSize'] = 20;
			$response['totalUser'] = $Email != null ? 1 : $q1->num_rows();
			if ($Email == null) {
				$response['totalOffering'] = $q->num_rows();
			}
			$response['data'] = $Email != null ? $q->result()[0] : $q->result();
			header('Content-Type: application/json; charset=utf-8');
			echo json_encode($response);exit();
		} else {
			$response['success'] = false;
	...
	public function fetch_Offering() {
		...
		if ($fromDate != null && $toDate != null) {
			$q = $this->db->select('Offerings.*, Users.Email as Email')
				->where('Offerings.templeId like', '%'.$templeId == null ? "" : $templeId.'%')
				->where('Offerings.FirstName like', '%'.$name == null ? "" : $name.'%')
				->join('Users', ' Users.id = Offerings.UserId', 'left')
				->where('Users.Email like', '%'.$email == null ? "" : $email.'%')
				->where('Offerings.CreatedOn >=', $fromDate)
				->where('Offerings.CreatedOn <=', $toDate)
				->group_by('Offerings.id')
				->get('Offerings');
		...
		if($q->num_rows() > 0) {
			...
			$response['success'] = false;
			$response['message'] = 'No Offering Found.';
			header('Content-Type: application/json; charset=utf-8');
			echo json_encode($response);exit();
		}
	}
	...
	public function export_Offering() {
		$files = glob($_SERVER['DOCUMENT_ROOT'].'/Offering/*', GLOB_BRACE);
		$zip = new ZipArchive;
		if ($zip->open($_SERVER['DOCUMENT_ROOT'].'/Offering.zip', ZipArchive::CREATE) === TRUE)
		{
			foreach($files as $file) {
				$files1 = glob($file.'/*', GLOB_BRACE);
				foreach($files1 as $file1) {
					$files2 = glob($file1.'/*', GLOB_BRACE);
					foreach($files2 as $file2) {
						if ($handle = opendir($file2.'/')) {
							// Add all files inside the directory
							while (false !== ($entry = readdir($handle)))
							{
							    if ($entry != "." && $entry != ".." && !is_dir($file2.'/'. $entry))
							    {
							        $zip->addFile($file2.'/'. $entry);
							    }
							}
							closedir($handle);
						}
		...
		$name= $_SERVER['DOCUMENT_ROOT'].'/Offering.zip';
